...
for a = 1:numConditions
    ...
    rheoIdx = eStack.Conditions{2,a}.rheoIdx;
    rheo_data = eStack.Conditions{2,a}.storedFits{1,rheoIdx};

    
    if rheoIdx>1    
        [pks,locs,w,p] = findpeaks(rheo_data,'MinPeakHeight',eStack.Conditions{2,a}.rheoAmp-0.0001);
        ...
        eStack.Conditions{2,a}.rheoFWHM = w(1)*(1/10)*(0.01/1);
    ...
    for b = 1:numel(IndexRelevant)
        ...
        eStack.Conditions{2,a}.fwhmAPs{IndexRelevant(b)}=w*(1/10); %milliseconds
       ...
       eStack.Conditions{2,a}.firingRate{IndexRelevant(b)}=eStack.Conditions{2,a}.numAPs{IndexRelevant(b)}/((numFrames)*(1/10)*(1/1000));
    end
end
